...
class SettingZoneScene: SKScene {
    
    let screenHeight = UIScreen.main.bounds.size.height
    
    var back:  SKSpriteNode = SKSpriteNode(imageNamed: "back")
    var zone23Icon:  SKSpriteNode = SKSpriteNode(imageNamed: "23ZoneIcon")
    var zone32Icon:  SKSpriteNode = SKSpriteNode(imageNamed: "32ZoneIcon")
    ...
    override func didMove(to view: SKView) {
        ...
        zone23Icon.position = CGPoint(x: SKVIEW_WIDTH*0.33, y: SKVIEW_HEIGHT-150)
        zone23Icon.size = CGSize(width: 100, height: 100)
        self.addChild(zone23Icon)
        
        zone32Icon.position = CGPoint(x: SKVIEW_WIDTH*0.66, y: SKVIEW_HEIGHT-150)
        zone32Icon.size = CGSize(width: 100, height: 100)
        self.addChild(zone32Icon)
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?)
    {
       ...
       if nodes.contains(back)
        ...
        else if nodes.contains(zone23Icon)
        {
            openZone23()
        }
        else if nodes.contains(zone32Icon)
        {
            openZone32()
        }
